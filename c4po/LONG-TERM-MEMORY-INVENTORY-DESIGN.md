...
sources, non-`auto_safe` tiers, and every unknown action or field. It does not
create or delete a target. This deliberately excludes retrieval indexes: no
...
original generated bytes and hashes. Rollback restores only the same allowlisted
generated targets, refuses to clobber a post-apply edit, verifies the receipt's
original hash against the applied plan, and is idempotent. If apply fails after
any replacement, the helper restores completed writes immediately and marks the
receipt rolled back.
