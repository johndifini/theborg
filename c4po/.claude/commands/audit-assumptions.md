...
3. In **STAGE 5 — SELECT**, keep the scheduled run's bounded ordinary selection
   policy unless the user requests a narrower cohort. There is no
   bulk first-review campaign: a registered draft remains honestly marked as a
   draft until an ordinary selected review reads it and supplies the judgment
   fields.
