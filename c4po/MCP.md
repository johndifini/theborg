...
| `vercel@claude-plugins-official` (GitHub `vercel/vercel-plugin`) | `vercel` (HTTP, `https://mcp.vercel.com`, OAuth, read-only in initial release) | plugin 0.49.0 @ `11c3258` (reviewed 2026-09-13) | Installed 2026-09-07 at **project scope only** (`ari/career-dossier`), for that project's Vercel deployment surface. Inert here: the plugin manifest has no `mcpServers` key, so Claude Code never registers the server, and no `.mcp.json` exists anywhere in the workspace. Ships **5 hooks** — 3 SessionStart, 1 SessionEnd (gated on Vercel/Next.js markers or an empty directory) and, since 0.49.0, a **PostToolUse hook** on `matcher: Skill` (`hooks/posttooluse-skill-telemetry.mjs`) that re-invokes itself detached to POST skill-usage events. Reviewed at source 2026-09-13: it honours `VERCEL_PLUGIN_TELEMETRY=off` (set in the project's `.claude/settings.json`), sends only plugin-shipped skill slugs plus a plugin-minted UUID — never the harness session id, which is used only as a local temp-file key — and the endpoint is unchanged from 0.48.0 (`telemetry.vercel.com`, the only outbound host in the hook tree). Subprocess calls remain fixed-arg and shell-free: `vercel --version`, `npm view vercel version`, and a self-spawn of the sender. **NOTE: 0.48.0 → 0.49.0 was an automatic update that bypassed review (security-audit findings 24/26); `gitCommitSha` did NOT change across it, so diff the VERSION, never the sha.** Promote to **Approved servers** if the MCP server is ever registered. |
