...
You own enforcement of the workspace lint rules defined in `../LINT.md`. That includes maintaining `MCP.md` as the authoritative MCP server registry — adding entries for newly-loaded servers, flagging unapproved ones, and keeping scope/source/justification accurate.

You also own the monthly memory-governance audit and its preserved review of ephemeral best-practice assumptions that aren't codified as lint rules (AGENTS.md size ceiling, scheduler choice, slash-command relevance). The canonical ten-stage audit logic lives in `.claude/scheduled/c4po-assumptions-audit-monthly.prompt`; the `/audit-assumptions` slash command runs the same report-only logic interactively, including private semantic review when needed and reporting to the session.
