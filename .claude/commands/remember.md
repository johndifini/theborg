...
`<cwd-with-slashes-as-dashes>` = the absolute path of the current working directory with every `/` replaced by `-`. Example: cwd `/Users/user/theborg/bones` → `~/.claude/projects/-Users-john-theborg-bones/memory/MEMORY.md`.
