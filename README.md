...
| `/audit-assumptions` | c4po | Runs the ten-stage `c4po-assumptions-audit-monthly` memory-governance audit interactively: complete inventory and mechanical checks, bounded semantic review with retirement-trigger evaluation, private review when needed, then the preserved Assumptions A-H. It defaults to report-only. With exact opt-in `--apply`, it may transactionally repair only public generated rule/command bridges and the privacy-scanned generated snapshot through the allowlisted step-7 helper, with exact diffs and a retained rollback receipt; all other tiers still stop. It reports to the session, never writes the monthly gate state, and the scheduled prompt remains report-only. |
