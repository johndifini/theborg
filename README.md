...
| `c4po-cli-update` | Sun at 6:00 AM | Runs `codex update` and `claude update` without invoking a model. Codex needs the explicit update; Claude's native install already self-updates on use, so its explicit call is belt-and-braces. Attempts both even if one fails, appends command output and versions to `c4po/.claude/scheduled/logs/c4po-cli-update.log`, emails the result every run, and returns nonzero on an update or notification failure. This deterministic shell job is intentionally exempt from the `.prompt`↔slash-command pairing rule (`LINT.md` → Scheduled tasks). |
