...
- `MEMORY-INVENTORY.yaml` — The governance registry for durable memory: every instruction file, rule, command, scheduled prompt, and wiki page gets a record saying why it exists, who owns it, what it costs to load, and when it's next reviewed — so the workspace's accumulated context can be audited instead of just accumulating. Schema in `MEMORY-INVENTORY.schema.json`; `python3 .bin/build-memory-inventory.py validate` checks it and `python3 .bin/build-memory-inventory.py discover` enumerates what is actually on disk. Currently a representative subset while the inventory is being built out (see `c4po/LONG-TERM-MEMORY-INVENTORY-DESIGN.md`).
