...
| `/audit-assumptions` | c4po | Runs the ten-stage `c4po-assumptions-audit-monthly` memory-governance audit interactively: complete inventory and mechanical checks, bounded semantic review with retirement-trigger evaluation, private review when needed, then the preserved Assumptions A-H. It remains report-only, reports all verdicts to the session instead of email, and skips both generated-state writes so it never blocks or advances the scheduled run. |
