...
| `/audit-assumptions` | c4po | Runs the ten-stage `c4po-assumptions-audit-monthly` memory-governance audit interactively: complete inventory and mechanical checks, bounded semantic review with retirement-trigger evaluation, private review when needed, then the preserved Assumptions A-H. It defaults to report-only. Exact opt-in `--bootstrap-review` selects the temporary weekly 15-unit first-review cohort (at least 12 knowledge units when available) without enlarging scheduled runs; exact opt-in `--apply` separately permits only transactional public generated rule/command-bridge and privacy-scanned snapshot repairs through the allowlisted step-7 helper, with exact diffs and a retained rollback receipt. All other tiers still stop. It reports to the session, never writes the monthly gate state, and the scheduled prompt remains report-only. |
