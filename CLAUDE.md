...
- Every scheduled task (a launchd job under the `com.john.theborg.*` namespace, driven by a `.prompt` file) has a corresponding interactive slash command in the owning agent's `.claude/commands/`.
