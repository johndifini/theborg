...
- Each non-private row in `README.md`'s Slash Commands table is factually consistent with its canonical command body (and any scheduled prompt that body delegates to). Compare operational claims, especially which files are staged or written, what is skipped, argument-dependent behavior, approvals, and other side effects. Flag contradictions and stale mechanisms, not differences in wording, emphasis, detail, or style; this is not a description-rewriting exercise. This row/body check does not apply to the root `AGENTS.md` Lint summary, which only routes readers to this file and the audits rather than documenting command behavior.
- **Private items are exempt from README.** A scheduled task or slash command marked private is intentionally omitted from `README.md` and must not be flagged as a coverage gap. Mark a command private with `private: true` in its YAML frontmatter; mark a scheduled task private with a `<!-- Private: true -->` marker line at the top of its `.prompt`. A task registered through a gitignored `.private/scheduled-tasks/*.tasks` table is likewise private and omitted from README. Private items still obey every other applicable lint rule (including the prompt-driven scheduled-task↔command pairing, step-reference and override completeness, and paths) — privacy only excuses them from the public-facing README, not from consistency checks.
### Paths
...
### Scoped rules
- Situational, durable guidance lives in `<scope>/.claude/rules/*.md`, which Claude loads directly. Codex has no equivalent loader but scans `.agents/skills` from the working directory up to the repository root, so `.bin/sync-codex-rule-skills.sh` publishes each rule as `<scope>/.agents/skills/<name>/SKILL.md` — a generated **stub** that tells Codex to read the canonical rule file. The rule's directory is therefore its scope in both harnesses.
- The stub carries only `name:`, `description:`, and a pointer; a rule's text is never copied into a skill or an `AGENTS.md`. Symlinking the stub does not work: Codex follows a symlinked skill *folder* but ignores a folder whose `SKILL.md` is a symlink.
- After adding, renaming, or removing a rule, run `.bin/sync-codex-rule-skills.sh`; `.bin/sync-codex-rule-skills.sh --check` must pass. The bridge refuses to overwrite a `SKILL.md` it did not generate, and removes only its own orphans.
- Each rule's frontmatter carries `name:` (matching its filename, minus any `.local` suffix) and a `description:` written as a trigger — "When <situation>: <what to do>". Codex selects a skill on the description alone, so any `paths:`/`globs:` scoping must also be stated there in prose. Keep the `paths:`/`globs:` keys for Claude.
- A skill only loads when the model picks it. A rule that must fire on **every** applicable turn therefore also needs a one-line summary in the governing `AGENTS.md`, pointing at the rule file for the full text. Where the rule sits above a `repos/*` boundary, the repo's own `AGENTS.md` carries that line — a Codex session inside the repo never sees the workspace root.
- A gitignored `*.local.md` rule generates into a `.local`-suffixed skill directory (`<name>.local/`), which `.gitignore` excludes — its stub would otherwise publish machine-specific description text.
...
- Use checkout-relative placeholders such as `<workspace-root>` in tracked documentation and prompts. Keep machine-specific instructions in gitignored `.claude/rules/*.local.md` or `.claude/settings.local.json` files. Keep confidential domain context under the owning agent's gitignored `.private/` directory; tracked `.private.example/` content must be wholly synthetic.
