...
- `MEMORY-INVENTORY.yaml` — Authoritative governance registry for durable memory artifacts: why each exists, who owns it, how it enters context, when it is reviewed, and what an audit may change. Reference data, not always-on context: read it when adding, relocating, or retiring a durable instruction, rule, skill, command, scheduled prompt, or wiki page. Schema in `MEMORY-INVENTORY.schema.json`; validate with `.bin/build-memory-inventory.py validate`, and enumerate what exists on disk with `.bin/build-memory-inventory.py discover` (read-only; it reports coverage but does not yet enforce it).
