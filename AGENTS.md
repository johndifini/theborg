...
- `MEMORY-INVENTORY.yaml` — Authoritative governance registry for durable memory artifacts: why each exists, who owns it, how it enters context, when it is reviewed, and what an audit may change. Reference data, not always-on context: read it when adding, relocating, or retiring a durable instruction, rule, skill, command, scheduled prompt, or wiki page. Schema in `MEMORY-INVENTORY.schema.json`; validate with `.bin/build-memory-inventory.py validate`, and enumerate what exists on disk with `.bin/build-memory-inventory.py discover` (read-only; add `--require-coverage` to fail on an unregistered artifact, which is what the lint audit runs). Registering an artifact is not the same as reviewing its record; see `LINT.md` → Memory inventory.
