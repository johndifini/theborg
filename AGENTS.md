...
- `design/` — Design deliverables for The Borg repository itself, produced and owned by `jony-vibe/` (currently the GitHub social preview served on the repo's Open Graph card). Design work for a repository under `repos/` lives in that repository's own `design/` directory instead, never here.
...
- `MEMORY-INVENTORY.yaml` — Authoritative governance registry for durable memory artifacts: why each exists, who owns it, how it enters context, when it is reviewed, and what an audit may change. Reference data, not always-on context: read it when adding, relocating, or retiring a durable instruction, rule, skill, command, scheduled prompt, or wiki page. **The session that creates, moves, or retires an artifact must update its registry record (and every generated companion's record) in the same change, then make `validate` and `discover --require-coverage` pass; audits only detect omissions later.** New artifacts may be registered mechanically as drafts with `.bin/build-memory-inventory.py bootstrap --write --artifact <workspace-relative-path>` after bridge generation. Registration is not review; follow `.claude/rules/memory-inventory-authoring.md` and see `LINT.md` → Memory inventory.
